# Adapted from https://github.com/google-deepmind/graphcast/blob/main/graphcast/solar_radiation.py
...
# Returns a xr.DataArray containing yearly average TSI values with a `time`
# coordinate in units of years since 0000-1-1. E.g. 2023.5 corresponds to
# the middle of the year 2023.
TsiDataLoader = Callable[[], xr.DataArray]
...
def reference_tsi_data() -> xr.DataArray:
...
# def hres_tsi_data() -> xr.DataArray:
#   with open("total_solar_irradiance_CMIP6_47r1.nc", "rb") as f:
#     with xr.load_dataset(f, decode_times=False) as ds:
#       return ds["tsi"]
...
def get_tsi(timestamps: Sequence[_TimestampLike], tsi_data: xr.DataArray) -> np.ndarray:
...
def _get_orbital_parameters(j2000_days: np.ndarray) -> _OrbitalParameters:
...
_get_integrated_radiation_jitted = _get_integrated_radiation
# jax.jit(
...
def get_toa_incident_solar_radiation(
    ...
    latitude: np.ndarray,
    longitude: np.ndarray,
    tsi_data: xr.DataArray | None = None,
...
def get_toa_incident_solar_radiation_for_xarray(
    data_array_like: xr.DataArray | xr.Dataset,
    tsi_data: xr.DataArray | None = None,
    integration_period: _TimedeltaLike = _DEFAULT_INTEGRATION_PERIOD,
    num_integration_bins: int = _DEFAULT_NUM_INTEGRATION_BINS,
    use_jit: bool = False,
    ...
    Args:
      data_array_like: A xr.Dataset or xr.DataArray from which to take the time
    ...
    Returns:
      xr.DataArray with dimensions `(time, lat, lon)` if `data_array_like` had
    ...
    if "time" in data_array_like.dims:
        ...
        timestamps = [data_array_like.coords["time"].data.item()]
    ...
    if "time" in data_array_like.dims:
        output = xr.DataArray(radiation, dims=("time", "lat", "lon"))
    else:
        output = xr.DataArray(radiation[0], dims=("lat", "lon"))
    ...
    return output.assign_attrs(units="J/m^2")
