...
def extract_obspack_locs_from_xarray(ds, obs, grid="latlon4"):
    if grid.startswith("latlon"):
        obs_t = obs.sel(time=ds.time, method="nearest").dropna("cell", how="all", subset=["lat", "lon", "height"])
        ds_loc = ds.sel(lat=obs_t.lat, lon=obs_t.lon, method="nearest")

        height_idx = ((obs_t.height >= ds_loc.gph_bottom * 1000) & (obs_t.height <= ds_loc.gph_top * 1000)).argmax(
            "level"
        )  # ((1000*ds_loc.gph - obs_t.height) ** 2).argmin("level")
        height_idx = height_idx.assign_coords(height=("cell", height_idx.values)).compute()
        ...
        obs_t = obs.sel(time=ds.time, method="nearest").dropna("cell", subset=["lat", "lon", "height"])
